...
func (_q *IdentityQuery) QueryEvents() *EventQuery {
...
func (_q *IdentityQuery) WithEvents(opts ...func(*EventQuery)) *IdentityQuery {
	query := (&EventClient{config: _q.config}).Query()
...
//		GroupBy(identity.FieldPlatform).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (_q *IdentityQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Identity, error) {
	...
	_spec.Node.Schema = _q.schemaConfig.Identity
	...
	if query := _q.withEvents; query != nil {
		if err := _q.loadEvents(ctx, query, nodes,
			func(n *Identity) { n.Edges.Events = []*Event{} },
			func(n *Identity, e *Event) { n.Edges.Events = append(n.Edges.Events, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *IdentityQuery) loadEvents(ctx context.Context, query *EventQuery, nodes []*Identity, init func(*Identity), assign func(*Identity, *Event)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(identity.EventsTable)
		joinT.Schema(_q.schemaConfig.IdentityEvents)
		s.Join(joinT).On(s.C(event.FieldID), joinT.C(identity.EventsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(identity.EventsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(identity.EventsPrimaryKey[0]))
...
func (_q *IdentityQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	t1.Schema(_q.schemaConfig.Identity)
	ctx = internal.NewSchemaConfigContext(ctx, _q.schemaConfig)
...
func (_q *IdentityQuery) ForUpdate(opts ...sql.LockOption) *IdentityQuery {
...
func (_q *IdentityQuery) ForShare(opts ...sql.LockOption) *IdentityQuery {
	if _q.driver.Dialect() == dialect.Postgres {
		_q.Unique(false)
	}
	_q.modifiers = append(_q.modifiers, func(s *sql.Selector) {
